...
s   *
  ...
  */
   ...
   *			radio.writeFast(&buf,32);
   *			radio.writeFast(&buf,32);   //Fills the FIFO buffers up
   *			bool ok = txStandBy(1000, 0);  //Returns 0 if failed after 1 second of retries. 1 if success.
   ...
   * @return True if the payload was delivered successfully false if not
   */
  void rf24_startFastWrite( void* buf, uint8_t len, bool multicast, bool startTx);
  ...
  */
  //#if defined (FAILURE_HANDLING)
    bool rf24_failureDetected(void); 
  //#endif
  ...
  /**
   * xMega SPI full duplex write read function
   *
   * @param reg Which register to read (the write byte)
   * @return Simultanious read from nrf24 (status byte)
   */
  uint8_t rf24_spi_writeread(SPI_t *spi, uint8_t reg);
   ...
   * @return Current value of status register
   */
  uint8_t rf24_write_payload(void* buf, uint8_t len, uint8_t writeType);
